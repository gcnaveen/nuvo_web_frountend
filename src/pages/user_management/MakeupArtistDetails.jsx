...
const STATUS_CONFIG = {
  ...
  ONEVENT:  { label: "On Event", badge: "warning",   icon: "bi-calendar-event-fill" },
  INACTIVE: { label: "Inactive", badge: "secondary", icon: "bi-dash-circle-fill" },
...
const SPECIALITIES = [
  "Bridal", "HD Makeup", "Airbrush", "Editorial",
  "Special FX", "Theatrical", "Fashion", "Film & TV", "Other",
];
...
const EditInput = ({ label, name, value, onChange, type = "text", placeholder = "" }) => (
  <div>
    <label className="md-label">{label}</label>
    <input type={type} name={name} className="md-input" value={value ?? ""} onChange={onChange} placeholder={placeholder} />
  </div>
);
...
const EditSelect = ({ label, name, value, onChange, options }) => (
  <div>
    <label className="md-label">{label}</label>
    <select name={name} className="md-select" value={value ?? ""} onChange={onChange}>
...
export default function MakeupArtistDetails() {
  ...
  const cancelEdit = () => { setIsEditing(false); setDraft({}); setSaveError(""); };
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await updateMua(mua.id, {
        ...
        experience_in_years: draft.experience_in_years !== "" ? Number(draft.experience_in_years) : null,
        status:              draft.status,
      });
  ...
  const handleAvatarUpload = async () => {
    ...
    try {
      ...
      setMua((prev) => ({ ...prev, profile_picture: res.data.data.profile_picture }));
      setAvatarFile(null);
  ...
  const handleGalleryUpload = async () => {
    ...
    try {
      ...
      const res = await uploadMuaImages(mua.id, fd);
      setMua((prev) => ({ ...prev, gallery_images: res.data.data.gallery_images }));
      setGalleryFiles([]);
      setGalleryPreviews([]);
      if (galleryInputRef.current) galleryInputRef.current.value = "";
      showToast(`${galleryFiles.length} image${galleryFiles.length > 1 ? "s" : ""} added!`);
    } catch (err) {
      showToast(err.response?.data?.message || "Upload failed.", "danger");
    } finally {
  ...
  if (loading) return (
    <div className="d-flex align-items-center justify-content-center" style={{ minHeight: 400 }}>
      <div className="text-center">
        <div className="spinner-border mb-3" style={{ width: 44, height: 44, color: ACCENT }} />
  ...
  return (
    <>
      ...
      <div className="page-heading">
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
          ...
          <div className="d-flex gap-2">
            {!isEditing ? (
              <>
                <button className="btn btn-outline-danger px-3" data-bs-toggle="modal" data-bs-target="#deleteMuaModal">
                  <i className="bi bi-trash me-1"></i>Delete
                </button>
                <button className="btn px-4" style={{ background: ACCENT, color: "#fff", border: "none" }} onClick={startEdit}>
      ...
      <div className="page-content">
        {saveError && (
          <div className="alert alert-danger py-2 mb-3">
            <i className="bi bi-exclamation-circle me-2"></i>{saveError}
          </div>
        )}
        ...
        <div className="row g-4">
          ...
          <div className="col-lg-4">
            ...
            <div className="md-card">
              <div className="md-card-bd text-center">
                ...
                {/* Clickable avatar */}
                <div className="md-avatar-wrap mb-3" onClick={() => avatarInputRef.current?.click()} title="Click to change photo">
                  {displayPic ? (
                    <img
                      src={displayPic}
                      alt={mua.full_name}
                      className="rounded-circle"
                      style={{ width: 110, height: 110, objectFit: "cover", border: `4px solid ${ACCENT}` }}
                    />
                  ) : (
                    <div
                      className="rounded-circle d-flex align-items-center justify-content-center fw-bold text-white mx-auto"
                      style={{ width: 110, height: 110, background: `linear-gradient(135deg, ${ACCENT}, #9c27b0)`, fontSize: "2rem", border: `4px solid ${ACCENT}55` }}
                    >
                      {initials(isEditing ? draft.full_name : mua.full_name)}
                    </div>
                  )}
                  <div className="md-avatar-overlay"><i className="bi bi-camera-fill"></i></div>
                  <span className={`badge bg-${statusCfg.badge} position-absolute bottom-0 end-0`} style={{ fontSize: ".7rem", padding: "4px 8px" }}>
                    {statusCfg.label}
                  </span>
                </div>

                <input type="file" ref={avatarInputRef} className="d-none" accept="image/*" onChange={handleAvatarSelect} />

                {avatarFile && (
                  <>
                    <div className="d-flex gap-2 justify-content-center mb-2">
                      <button className="btn btn-sm btn-success px-3" onClick={handleAvatarUpload} disabled={uploadingAvatar}>
                        {uploadingAvatar
                          ? <><span className="spinner-border spinner-border-sm me-1" />Uploading…</>
                          : <><i className="bi bi-cloud-upload me-1"></i>Save Photo</>
                        }
                      </button>
                      <button className="btn btn-sm btn-light" onClick={cancelAvatarPreview} disabled={uploadingAvatar}>Cancel</button>
                    </div>
                    <p className="text-muted mb-3" style={{ fontSize: ".72rem" }}>
                      <i className="bi bi-info-circle me-1 text-primary"></i>Preview — click Save Photo to upload
                    </p>
                  </>
                )}

                <h5 className="fw-bold mb-1">{isEditing ? draft.full_name || "—" : mua.full_name}</h5>
                <div className="mb-3">
                  {(isEditing ? draft.makeup_speciality : mua.makeup_speciality) && (
                ...
                <div className="row g-2">
                  <div className="col-6">
                    <div className="md-stat">
                      <div className="md-stat-val">{(isEditing ? draft.experience_in_years : mua.experience_in_years) ?? "—"}</div>
                      <div className="md-stat-lbl">Yrs Exp</div>
                    </div>
                  </div>
                  <div className="col-6">
                    <div className="md-stat">
                      <div className="md-stat-val" style={{ fontSize: ".85rem" }}>{(isEditing ? draft.city : mua.city) || "—"}</div>
            ...
            <div className="md-card">
              ...
              <div className="md-card-bd">
                <div className="row g-3">
                  <div className="col-12"><Field label="Profile ID" value={mua.id} /></div>
                  <div className="col-12"><Field label="User ID"    value={mua.user_id} /></div>
                  <div className="col-12"><Field label="Email"      value={mua.email} /></div>
                  <div className="col-12"><Field label="Phone"      value={mua.phone_number} /></div>
                  <div className="col-12"><Field label="Joined"     value={fmtDate(mua.joined_date)} /></div>
                  <div className="col-12">
                    <label className="md-label">Account Status</label>
                    {isEditing ? (
                      <EditSelect name="status" value={draft.status} onChange={handleDraftChange}
                        options={STATUSES.map((s) => ({ value: s, label: STATUS_CONFIG[s]?.label || s }))} />
                    ) : (
                      <div className="md-value">
                        <span className={`badge bg-${statusCfg.badge} px-3 py-2`} style={{ fontSize: ".8rem" }}>
                          <i className={`bi ${statusCfg.icon} me-1`}></i>{statusCfg.label}
                        </span>
                      </div>
                    )}
                  </div>
          ...
          <div className="col-lg-8">
            ...
            <div className="md-card">
              <div className="md-card-hd">
                <i className="bi bi-person" style={{ color: ACCENT }}></i>
                  ...
                  {isEditing ? (
                    <>
                      <div className="col-md-6">
                        <EditInput label="Full Name *" name="full_name" value={draft.full_name} onChange={handleDraftChange} placeholder="Full name" />
                      </div>
                      <div className="col-md-6">
                        <EditSelect label="Gender" name="gender" value={draft.gender} onChange={handleDraftChange}
                          options={[{ value: "", label: "— Select —" }, ...GENDERS.map((g) => ({ value: g, label: g }))]} />
                      </div>
                      <div className="col-md-6">
                        <EditSelect label="Makeup Speciality" name="makeup_speciality" value={draft.makeup_speciality} onChange={handleDraftChange}
                          options={[{ value: "", label: "— Select —" }, ...SPECIALITIES.map((s) => ({ value: s, label: s }))]} />
                      </div>
                      <div className="col-md-6">
                        <EditInput label="Experience (Years)" name="experience_in_years" type="number" value={draft.experience_in_years} onChange={handleDraftChange} placeholder="0" />
                      </div>
                      <div className="col-md-6">
                        <EditInput label="City"    name="city"    value={draft.city}    onChange={handleDraftChange} placeholder="City" />
                      </div>
                      <div className="col-md-6">
                        <EditInput label="State"   name="state"   value={draft.state}   onChange={handleDraftChange} placeholder="State" />
                      </div>
                      <div className="col-md-6">
                        <EditInput label="Country" name="country" value={draft.country} onChange={handleDraftChange} placeholder="Country" />
                      </div>
                    </>
                  ) : (
                    <>
                      <div className="col-md-6"><Field label="Full Name"         value={mua.full_name} /></div>
                      <div className="col-md-6"><Field label="Gender"            value={mua.gender} /></div>
                      <div className="col-md-6"><Field label="Makeup Speciality" value={mua.makeup_speciality} /></div>
                      <div className="col-md-6"><Field label="Experience"        value={mua.experience_in_years != null ? `${mua.experience_in_years} years` : null} /></div>
                      <div className="col-md-6"><Field label="City"              value={mua.city} /></div>
                      <div className="col-md-6"><Field label="State"             value={mua.state} /></div>
                      <div className="col-md-6"><Field label="Country"           value={mua.country} /></div>
                      <div className="col-md-6"><Field label="Joined"            value={fmtDate(mua.joined_date)} /></div>
                    </>
                  )}
                </div>
              </div>
            ...
            <div className="md-card">
              <div className="md-card-hd">
                <i className="bi bi-images" style={{ color: ACCENT }}></i>
                <h6>Work Portfolio</h6>
                <span className="ms-auto badge bg-light text-muted border" style={{ fontSize: ".72rem" }}>
              ...
              <div className="md-card-bd">
                ...
                {galleryPreviews.length > 0 && (
                  <div className="mb-3 p-3 rounded-3" style={{ background: "#fce4f0", border: `1.5px dashed ${ACCENT}` }}>
                    <div className="d-flex justify-content-between align-items-center mb-2">
                      <span className="small fw-bold" style={{ color: ACCENT }}>
                        <i className="bi bi-cloud-upload me-1"></i>
                        {galleryPreviews.length} image{galleryPreviews.length > 1 ? "s" : ""} ready to upload
                      </span>
                      <div className="d-flex gap-2">
                        <button className="btn btn-sm btn-success px-3" onClick={handleGalleryUpload} disabled={uploadingGallery}>
                          {uploadingGallery
                            ? <><span className="spinner-border spinner-border-sm me-1" />Uploading…</>
                            : <><i className="bi bi-cloud-upload me-1"></i>Upload All</>
                          }
                        </button>
                        <button className="btn btn-sm btn-light" onClick={cancelGalleryPreview} disabled={uploadingGallery}>Cancel</button>
                      </div>
                    </div>
                    <div className="row g-2">
                      ...
                      <div className="col-md-4 col-6" key={idx}>
                        <div className="md-gallery-item">
                          <img
                            ...
                            onError={(e) => { e.target.closest(".md-gallery-item").style.display = "none"; }}
                          />
                          <button
                ...
                {!galleryPreviews.length && (
                  <div className="mt-3 pt-3 border-top d-flex justify-content-end">
                    <button
                      ...
                      style={{ border: `1.5px solid ${ACCENT}`, color: ACCENT, borderRadius: 8 }}
                ...
                <input type="file" ref={galleryInputRef} className="d-none" accept="image/*" multiple onChange={handleGallerySelect} />
              </div>
            </div>
      ...
      {/* ── DELETE MODAL ──────────────────────────────────────── */}
      <div className="modal fade" id="deleteMuaModal" tabIndex="-1" aria-hidden="true">
          ...
          <div className="modal-content shadow-lg" style={{ borderRadius: 14 }}>
            <div className="modal-body p-4 text-center">
              ...
              <p className="text-muted mb-0" style={{ fontSize: ".9rem" }}>
                You are about to permanently delete <strong>{mua?.full_name}</strong>.
                This removes their profile, account, and all portfolio images. This cannot be undone.
              </p>
              {deleteError && (
                <div className="alert alert-danger py-2 mt-3 mb-0">
                  <i className="bi bi-exclamation-circle me-2"></i>{deleteError}
                </div>
              )}
            </div>
            <div className="modal-footer border-0 px-4 pb-4 pt-0 d-flex gap-2">
              <button className="btn btn-light flex-fill" data-bs-dismiss="modal" disabled={deleting}>Cancel</button>
              <button className="btn btn-danger flex-fill" onClick={handleDelete} disabled={deleting}>
                {deleting
